...
cat > tax_invoice_th/__init__.py << 'EOL'
from . import models
EOL

# Create models __init__.py file
cat > tax_invoice_th/models/__init__.py << 'EOL'
...
access_account_move_tax,account.move.tax,account.model_account_move,account.group_account_invoice,1,1,1,1
EOL

# Make the directory readable
chmod -R 755 tax_invoice_th
